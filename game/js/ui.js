...
// =========================================
// UTILIDADES Y CÁLCULOS BASE (Protegidos contra valores nulos)
// =========================================
export function getMapScale() { return 1 + (((gameState.mapLevel || 1) - 1) * 0.5); }
export function getMaxHp() { return (gameState.player.baseMaxHp || 0) + (gameState.player.armor ? (gameState.player.armor.hpBonus || 0) : 0); }
export function getMaxMp() { return (gameState.player.baseMaxMp || 0) + (gameState.player.armor ? (gameState.player.armor.mpBonus || 0) : 0); }
export function getMaxEp() { return (gameState.player.baseMaxEp || 0); }
export function getAtk() { return (gameState.player.baseAd || 0) + (gameState.player.weapon ? (gameState.player.weapon.atk || 0) : 0); }
export function getDef() { return (gameState.player.baseArmor || 0) + (gameState.player.armor ? (gameState.player.armor.def || 0) : 0); }
export function getMag() { return (gameState.player.baseAp || 0) + (gameState.player.weapon ? (gameState.player.weapon.mag || 0) : 0); }
...
export function updateHUD() {
    try {
        ...
        console.error("Error interno en updateHUD:", e);
    }
}
...
export function openStatsModal() {
    try {
        ...
        const finalHp = getMaxHp() || 0;
        const finalMp = getMaxMp() || 0;
        const finalAd = getAtk() || 0;
        const finalAp = getMag() || 0;
        const finalArmor = getDef() || 0;
        
        // Protegemos el toFixed para que no crashee si el valor es incorrecto
        let atkSpeed = Number(p.baseAttackSpeed);
        if (isNaN(atkSpeed)) atkSpeed = 1.0;
