...
function openNPCModal() {
    ...
    // El tamaño de esta imagen ahora lo controla estrictamente el CSS
...
function castSpell(spellName) {
    try {
        ...
        if (spellName === 'fuego') {
            if (player.mp < 12) { playSFX(sfx.error); logMsg("¡Faltan 12 MP para Fuego!"); return; } 
            ...
            if (player.mp < 15) { playSFX(sfx.error); logMsg("¡Faltan 15 MP para Curar!"); return; } 
...
/* FIX PC/CRASH: Reforzada la seguridad del desbloqueo de botones en el turno enemigo */
function processEnemyTurn(enemy) {
    let turnCompletedNormally = false;
    try {
        if (!inCombat || !currentEnemyTile) return; // Seguridad si se huye rápido
        ...
        console.error("Error turno enemigo crítico detectado:", e); 
        logCombat("<b style="color:red">⚠️ Error en turno enemigo. Forzando desbloqueo.</b>");
    } 
    finally { 
        // Desbloqueo estricto con temporizador
        setTimeout(() => { 
            lockCombatButtons(false); 
            if (player.hp <= 0) { 
...
// Fix para 100vh real en móviles
...
function setupTouchControls() {
    const bindTouch = (id, dx, dy) => {
        ...
        // touchstart es ideal para dispositivos móviles, es más rápido que un 'click'
...
// Inicializar los controles táctiles
setupTouchControls();