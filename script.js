...
function loadGameBtn() {
    ...
    try {
        ...
        if (savedString) {
            ...
            // CHEQUEO DE SEGURIDAD (Evita error de pantalla negra por mapa viejo)
            if (!saveData.mapDataState || saveData.mapDataState.length !== 10000) {
                console.log("Archivo de guardado antiguo. Creando mundo nuevo.");
                return false;
            }
...
function selectClass(className) {
    ...
    if (className === 'Guerrero') {
        player.baseMaxHp = 60; player.hp = 60; player.baseMaxMp = 10; player.mp = 10;
        ...
        player.baseMaxHp = 35; player.hp = 35; player.baseMaxMp = 40; player.mp = 40;
...
/* =========================================
   NUEVO RENDER DE CAMARA ABSOLUTA (NO CRASHEA)
   ========================================= */
function render() {
    ...
    // Solo se dibuja en un radio de 2 casillas a la redonda (Suficiente para pantallas).
    const vRadius = 2; 
    ...
    for(let y = Math.max(0, player.y - vRadius); y <= Math.min(MAP_H - 1, player.y + vRadius); y++) {
        for(let x = Math.max(0, player.x - vRadius); x <= Math.min(MAP_W - 1, player.x + vRadius); x++) {
            ...
            // EL MAGIA OCURRE AQUÍ: POSICIONAMIENTO MATEMÁTICO DIRECTO
...
function checkLevelUp() {
    ...
    if (player.xp >= xpNeeded) {
        ...
        if (player.playerClass === 'Guerrero') { player.baseMaxHp += 8; player.baseMaxMp += 2; player.baseAtk += 2; player.baseDef += 1; } 
        else { player.baseMaxHp += 4; player.baseMaxMp += 8; player.baseMag += 2; }
        player.hp = Math.min(getMaxHp(), player.hp + 8); player.mp = Math.min(getMaxMp(), player.mp + 8);
        playSFX(sfx.level_up); saveGame();
    }
}
