...
function logMsg(t) { 
    ...
    logs.forEach(log => {
        if(log) {
            const d = document.createElement('div'); d.textContent = `> ${t}`; log.prepend(d); 
        }
    });
}
...
function loadGameBtn() {
    ...
    try {
        ...
        if (savedString) {
            ...
            if (!saveData.mapDataState || saveData.mapDataState.length !== 10000) return false;
...
// [NUEVO] Función para el D-PAD táctil en móviles
...
function castSpell(spellName) {
    ...
    if (spellName === 'fuego') {
        ...
        const heal = Math.floor(pMag * 2.5);
...
function processEnemyTurn(enemy) {
    let enemyMag = enemy.mag || 0; let isMagic = (enemyMag > 0 && Math.random() < 0.4); 
    let eDmg = 0; let minDmg = Math.max(1, Math.floor(enemy.atk * 0.15));
    ...
    if (isMagic) {
        ...
        if (enemy.isBoss && enemy.trait === 'crit' && Math.random() < 0.3) { eDmg = Math.floor(eDmg * 1.5); logCombat(`⚡ <b style="color:#ffeb3b">¡GOLPE CRÍTICO!</b>`); }
...
// [NUEVO] Evento para recalcular la cámara si se cambia el tamaño de la pantalla (PC o girar el móvil)
